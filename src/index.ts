...
const createArrow = (prevButtonClass: string, nextButtonClass: string) =>
    `<span class="${styles.button} ${styles['prev-button']} ${prevButtonClass}"></span><span class="${styles.button} ${styles['next-button']} ${nextButtonClass}"></span>`;
...
class Carousel {
    ...
    constructor(imgList: string[], options: CarouselOption) {
        ...
        this.dots = Array.from(document.querySelectorAll(`${element} .${styles.dot}`));
        this.sliders = Array.from(document.querySelectorAll(`${element} .${styles.slider}`));
